...
impl OvBackend {
    ...
    pub fn with_threads(model_path: &str, threads: InferenceThreads) -> Result<Self, BackendError> {
        ...
        // frontend.
        let model = core.read_model_from_file(model_path, "").map_err(|e| {
            BackendError::InitializationError(format!(
                "openvino read_model({model_path}) failed: {e:?}"
            ))
        })?;

        let compiled = core.compile_model(&model, DeviceType::CPU).map_err(|e| {
            BackendError::InitializationError(format!("openvino compile_model(CPU) failed: {e:?}"))
        })?;
        ...
        if input_specs.is_empty() {
            return Err(BackendError::InitializationError(format!(
                "openvino: model {model_path} has zero inputs"
            )));
        }
...
fn introspect_inputs(compiled: &CompiledModel) -> Result<Vec<NodeSpec>, BackendError> {
    ...
    for i in 0..count {
        let node = compiled.get_input_by_index(i).map_err(|e| {
            BackendError::InitializationError(format!(
                "openvino get_input_by_index({i}) failed: {e:?}"
            ))
        })?;
        ...
        let shape = shape_to_vec(&node.get_shape().map_err(|e| {
            BackendError::InitializationError(format!(
                "openvino input[{i}].get_shape failed: {e:?}"
            ))
        })?);
...
fn introspect_outputs(compiled: &CompiledModel) -> Result<Vec<NodeSpec>, BackendError> {
    ...
    for i in 0..count {
        let node = compiled.get_output_by_index(i).map_err(|e| {
            BackendError::InitializationError(format!(
                "openvino get_output_by_index({i}) failed: {e:?}"
            ))
        })?;
        let name = node.get_name().map_err(|e| {
            BackendError::InitializationError(format!(
                "openvino output[{i}].get_name failed: {e:?}"
            ))
        })?;
        let shape = shape_to_vec(&node.get_shape().map_err(|e| {
            BackendError::InitializationError(format!(
                "openvino output[{i}].get_shape failed: {e:?}"
            ))
        })?);
        specs.push(NodeSpec { name, shape });
    }
...
impl InferenceBackend for OvBackend {
    ...
    fn run(
        ...
        let mut state = self.state.lock().map_err(|e| {
            BackendError::ExecutionFailure(format!("openvino state lock poisoned: {e}"))
        })?;
        ...
        let mut request = state.compiled.create_infer_request().map_err(|e| {
            BackendError::ExecutionFailure(format!("openvino create_infer_request failed: {e:?}"))
        })?;
        ...
        for (idx, spec) in self.input_specs.iter().enumerate() {
            let storage = inputs.named_tensors.get(&spec.name).ok_or_else(|| {
                BackendError::ExecutionFailure(format!(
                    "openvino: missing input tensor '{}'",
                    spec.name
                ))
            })?;
            ...
            let shape_obj = Shape::new(&spec.shape.iter().map(|&d| d as i64).collect::<Vec<_>>())
                .map_err(|e| {
                BackendError::ExecutionFailure(format!(
                    "openvino Shape::new({:?}) failed: {e:?}",
                    spec.shape
                ))
            })?;
            let mut tensor = Tensor::new(ElementType::F32, &shape_obj).map_err(|e| {
                BackendError::ExecutionFailure(format!("openvino Tensor::new failed: {e:?}"))
            })?;
            {
                let buf: &mut [f32] = tensor.get_data_mut::<f32>().map_err(|e| {
                    BackendError::ExecutionFailure(format!(
                        "openvino tensor get_data_mut failed: {e:?}"
                    ))
                })?;
                if buf.len() != raw.len() {
            ...
            request
                .set_input_tensor_by_index(idx, &tensor)
                .map_err(|e| {
                    BackendError::ExecutionFailure(format!(
                        "openvino set_input_tensor_by_index({idx}) failed: {e:?}"
                    ))
                })?;
        }
        ...
        request
            .infer()
            .map_err(|e| BackendError::ExecutionFailure(format!("openvino infer failed: {e:?}")))?;
        ...
        for (idx, spec) in self.output_specs.iter().enumerate() {
            let tensor = request.get_output_tensor_by_index(idx).map_err(|e| {
                BackendError::ExecutionFailure(format!(
                    "openvino get_output_tensor_by_index({idx}) failed: {e:?}"
                ))
            })?;
            let data = tensor.get_data::<f32>().map_err(|e| {
                BackendError::ExecutionFailure(format!(
                    "openvino output[{idx}].get_data::<f32> failed: {e:?}"
                ))
            })?;
