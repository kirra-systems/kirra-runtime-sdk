...
- **cmd_vel_interceptor** — intercepts the doer's proposal topic, enforces via Kirra, publishes to `/cmd_vel_safe`. The input is an **atomic evidence-bound proposal envelope** (canonical JSON in a `std_msgs/String`, see [`bound_proposal.py`](kirra_safety/bound_proposal.py)), not a bare `Twist`: one message carries the proposed velocities *and* the `release_binding` naming the perception frame, platform profile, and Occy proposal digest they were authored against, so command and evidence cannot go stale independently. A malformed, empty, or unbound envelope stops the robot and makes no request (fail-closed). When the verifier mints ADR-0033 release tokens, it also relays each signed frame (`payload(176)‖token(96)`, 272 bytes — the evidence-bound V2 payload) on `/kirra/release` for the verifying motor consumer (`robot/kirra_motor_consumer.py`) — pure carriage, strict parse, malformed release → nothing published (the consumer decels to zero)
...
- **occy_doer** — the DOER: drives the robot to a `/goal_pose` using Occy + Taj + KIRRA, publishing proposals to `/cmd_vel_raw` (see [`docs/testing/OCCY_DOER_BRIDGE.md`](../../../../docs/testing/OCCY_DOER_BRIDGE.md)). `scan_stale_s` is REQUIRED (no default — a safety number; `config/kirra_params.yaml` carries the deployment value): perception older than the budget holds the robot. Opt-in **frame-health detectors** ([`sensor_freshness.py`](kirra_safety/sensor_freshness.py), `frame_health_enabled`) additionally catch what an arrival-time budget structurally cannot — a lidar driver that has wedged and is republishing its last buffer at the full rate, which reads as perfectly fresh while the world it describes is frozen. They track producer **header-stamp progression**; content fingerprinting is corroborating evidence only, since a stationary robot facing a wall legitimately emits identical scans forever
