...
// `lg` breakpoint (where the desktop sidebar is hidden). The overlay is rendered
// through a portal to <body> — the top nav has `backdrop-blur`, which would
// otherwise become the containing block for `position: fixed` and shrink the
// overlay to the header. Closes on navigation, backdrop tap, and Escape.
export function MobileNav() {
  ...
  const overlay = (
    <div className="fixed inset-0 z-[100] lg:hidden" role="dialog" aria-modal="true" aria-label="Navigation">
      ...
      <aside className="absolute inset-y-0 left-0 flex w-[270px] max-w-[82%] flex-col border-r border-line bg-surface shadow-panel">
