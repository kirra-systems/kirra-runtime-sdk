...
export function TrajectoryPlot({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ height }} className="w-full" role="img" aria-label="bird's-eye trajectory comparison">
      ...
      <text x={sx(goalX)} y={sy(halfWidth) - 0.8} fill="var(--c-safe)" fontSize={2} textAnchor="middle" fontFamily="monospace">goal</text>

      {/* obstacle */}
      {obstacleX !== undefined && (
        <>
          <rect x={sx(obstacleX) - 1.5} y={sy(0) - 1.5} width={3} height={3} rx={0.5} fill="rgba(255,84,104,0.9)" />
          <text x={sx(obstacleX)} y={sy(0) + 4.4} fill="var(--c-crit)" fontSize={2} textAnchor="middle" fontFamily="monospace">car</text>
        </>
      )}

      {/* ego start */}
      <circle cx={sx(egoX)} cy={sy(0)} r={1} fill="var(--c-ice)" />
      <text x={sx(egoX)} y={sy(0) - 2.2} fill="var(--c-ice)" fontSize={2} textAnchor="middle" fontFamily="monospace">ego</text>
