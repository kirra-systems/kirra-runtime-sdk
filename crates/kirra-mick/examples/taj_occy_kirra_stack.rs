...
fn main() {
    ...
    report(
        "off-centre obstacle, wide road",
        &o,
        "routes around (admitted)",
    );
    ...
    println!(
        "  Taj produced the corridor+objects; Mick the intent; Occy proposed; KIRRA disposed."
    );
    println!(
        "  Across every regime Occy only PROPOSES; KIRRA is the sole authority that admits or"
    );
    println!(
        "  refuses. This is exactly the System-2 stack that runs on the Orin NX 16GB (ADR-0014)."
    );
}
