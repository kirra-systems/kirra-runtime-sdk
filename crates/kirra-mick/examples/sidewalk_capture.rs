...
use kirra_capture_schema::{
    CaptureOutcome, CaptureRecord, CaptureSource, PoseSnapshot, TrajectoryCaptureExt,
};
use kirra_planner::{
...
fn pose_snap(p: &Pose) -> PoseSnapshot {
    PoseSnapshot {
        x_m: p.x_m,
        y_m: p.y_m,
...
fn main() {
    ...
    run_phase(
        ...
        |tick| {
            let y = if tick < 30 {
                ...
                0.30 * (tick as f64 - 30.0) * DT_S * 5.0
            };
    ...
    println!(
        "  supervised signal for tuning the DOER. The collector links kirra-capture-schema only."
    );
}
