...
fn left_turn(r: f64, half: f64) -> LaneGraph {
    ...
    LaneGraph::new()
        .with_lane(
            Lane::straight(1, 0.0, 0.0, 20.0, half, line, line)
...
fn the_planner_follows_a_gentle_junction_turn_and_kirra_admits() {
    ...
    assert_eq!(
        plan.kind,
        ProposalKind::Motion,
        "the ego drives the turn, not HOLD"
    );
    // It turns: heading climbs well past 45° toward the north (π/2) exit.
    assert!(
        max_heading > 1.0,
        "the path turns toward north, got max heading {max_heading} rad"
    );
    ...
    assert!(
        max_y > 0.6 * r,
        "the path climbs through the turn, got max_y {max_y} (arc top {r})"
    );
    assert!(
        matches!(
            verdict,
