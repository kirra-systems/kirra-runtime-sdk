...
fn ground_turn<'a>(
    g: &'a LaneGraph,
    ego_corr: &'a dyn kirra_trajectory::corridor::CorridorSource,
    goal: Pose,
    dir: TurnDirection,
) -> PlanOutput {
    let input = PlanInput {
...
fn mick_turn_left_grounds_through_the_junction_and_kirra_admits() {
    ...
    assert_eq!(
        plan.kind,
        ProposalKind::Motion,
        "TurnAt grounds to a drive, not HOLD"
    );
    ...
    assert!(
        max_heading > 1.0,
        "the path turns LEFT (toward north), got max heading {max_heading}"
    );
    assert!(
        max_y > 0.6 * R,
        "and climbs the left branch, got max_y {max_y}"
    );
    ...
    assert!(
        ...
        "KIRRA admits the turn, got {verdict:?}"
    );
}
...
fn mick_turn_right_picks_the_other_branch() {
    ...
    assert!(
        min_y < -0.6 * R,
        "RIGHT takes the south branch, got min_y {min_y}"
    );
}
...
fn a_turn_with_no_such_branch_fails_closed_to_hold() {
    ...
    assert_eq!(
        ...
        "no straight branch → fail-closed HOLD"
    );
