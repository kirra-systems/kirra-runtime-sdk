...
fn route_to_drives_the_whole_two_junction_route_kirra_bounding_throughout() {
    ...
    assert_eq!(
        route_ids,
        vec![1, 2, 3, 4, 5],
        "the router picks the left branch at J1, not the decoy"
    );
    ...
    let route: LaneCorridor = g
        .route_corridor(&route_ids, 0.95, 5)
        .expect("stitch the route corridor");
    ...
    assert!(
        ego_max_heading > 1.4,
        "the ego turns north through the first junction (≈π/2), got max heading {ego_max_heading}"
    );
    ...
    assert!(
        ego.pose.y_m > 45.0,
        "the ego reaches the final exit lane (y≈52), got y={}",
        ego.pose.y_m
    );
    assert!(
        ego.pose.x_m > 58.0,
        "and travels east along it past the second junction, got x={}",
        ego.pose.x_m
    );
}
