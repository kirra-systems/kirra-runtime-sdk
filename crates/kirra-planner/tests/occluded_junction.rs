...
fn the_ego_creeps_into_a_blind_junction_but_cruises_an_open_one() {
    ...
    let admit = |p: &kirra_planner::PlanOutput| {
        matches!(
    ...
    assert!(
        open_peak > 6.0,
        "an open junction is taken at cruise, got peak {open_peak}"
    );
    ...
    assert_eq!(
        blind.kind,
        ProposalKind::Motion,
        "the ego still proceeds (creeps), it does not HOLD"
    );
    let blind_peak = peak_speed(&blind);
    assert!(
        blind_peak < 5.0,
        "the blind approach is creep-capped to the assured-clear speed, got peak {blind_peak}"
    );
    ...
    let blinder = plan_for_intent(
        &mut GeometricPlanner::default(),
        &intent,
        &world(&corr, &blinder_g),
    );
    assert!(
        peak_speed(&blinder) < blind_peak,
        "less sight → slower creep ({} vs {blind_peak})",
        peak_speed(&blinder)
    );
}
...
fn occlusion_creep_is_driven_by_map_occluder_geometry_not_a_hand_fed_datum() {
    ...
    assert_eq!(
        derived_g.sight_distance(1),
        Some(5.0),
        "the sight distance is derived from the footprint"
    );

    let derived = plan_for_intent(
        &mut GeometricPlanner::default(),
        &intent,
        &world(&corr, &derived_g),
    );
    assert_eq!(
        derived.kind,
        ProposalKind::Motion,
        "the ego creeps the geometry-derived blind junction"
    );
    let derived_peak = peak_speed(&derived);

    // Byte-for-byte the same outcome as the hand-fed Some(5.0) datum: same cap, same creep.
    let handfed = plan_for_intent(
        &mut GeometricPlanner::default(),
        &intent,
        &world(&corr, &approach(Some(5.0))),
    );
    assert!(
        (derived_peak - peak_speed(&handfed)).abs() < 1e-9,
        "derived geometry == hand-fed datum"
    );
    assert!(
        derived_peak < 5.0,
        "creep-capped to the assured-clear speed, got peak {derived_peak}"
    );
    assert!(
        admit(&derived),
        "KIRRA admits the geometry-derived occlusion creep"
    );
    ...
    assert_eq!(
        open_g.sight_distance(1),
        None,
        "a distant footprint shadows nothing"
    );
    let open = plan_for_intent(
        &mut GeometricPlanner::default(),
        &intent,
...
fn occlusion_creep_composes_with_a_stop_sign_at_the_same_blind_junction() {
    ...
    assert!(
        peak_speed(&out) < 5.0,
        "the blind+stop approach stays under the creep cap, got peak {}",
        peak_speed(&out)
    );
    assert!(
        matches!(
