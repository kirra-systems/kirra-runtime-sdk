...
fn two_junction_route(occlude: bool) -> LaneGraph {
    ...
    let mut g = LaneGraph::new()
        ...
        .with_lane(lane(5, vec![Point { x_m: 54.0, y_m: 52.0 }, Point { x_m: 90.0, y_m: 52.0 }], 0.0, &[]))
...
fn run(g: &LaneGraph, objects: &[PerceivedObject]) -> RunResult {
    ...
    let goal = Pose { x_m: 84.0, y_m: 52.0, heading_rad: 0.0 };

    let mut driver = MickDriver::new(ScriptedBrain::new(vec![MickIntent::RouteTo { x_m: 84.0, y_m: 52.0 }; 200]));
    ...
    // already be above it).
    let mut ego = EgoState { pose: Pose { x_m: 16.0, y_m: 0.0, heading_rad: 0.0 }, linear_x_mps: 2.0, yaw_rate_rads: 0.0, stamp_ms: 0 };
    ...
    for tick in 1..=TICKS {
        ...
        if replan_due {
            ...
            let v = validate_trajectory_slow(&plan.trajectory, &route, objects, &VehicleConfig::default_urban(), None, FleetPosture::Nominal);
            total += 1;
...
fn the_full_stack_routes_two_junctions_creeping_the_occluded_approach_kirra_bounding() {
    ...
    assert!(occluded.admitted > 0, "KIRRA admitted the trajectories the loop tracked");
    ...
    assert!(occluded.approach_peak_mps < 3.5, "occluded approach is creep-capped, got {}", occluded.approach_peak_mps);
    assert!(occluded.approach_peak_mps < open.approach_peak_mps - 0.8,
        "occluded approach is slower than the open one ({} vs {})", occluded.approach_peak_mps, open.approach_peak_mps);
    ...
    assert!(occluded.reached_lane5, "the ego routes through both junctions to lane 5, final=({:.1},{:.1})", occluded.ego_final.x_m, occluded.ego_final.y_m);
}
...
fn the_full_stack_follows_a_dynamic_lead_in_the_final_lane_and_kirra_admits() {
    ...
    let ego = EgoState { pose: Pose { x_m: 58.0, y_m: 52.0, heading_rad: 0.0 }, linear_x_mps: 5.0, yaw_rate_rads: 0.0, stamp_ms: 0 };
    let goal = Pose { x_m: 84.0, y_m: 52.0, heading_rad: 0.0 };
    let lead = [PerceivedObject { id: 9, pos: Point { x_m: 68.0, y_m: 52.0 }, velocity_mps: 1.5, heading_rad: 0.0, vel: Point { x_m: 1.5, y_m: 0.0 } }];
    let intent = MickIntent::RouteTo { x_m: 84.0, y_m: 52.0 };
    let peak = |p: &PlanOutput| p.trajectory.iter().map(|t| t.velocity_mps).fold(0.0, f64::max);
    let admit = |p: &PlanOutput, objs: &[PerceivedObject]| matches!(
        validate_trajectory_slow(&p.trajectory, &route, objs, &VehicleConfig::default_urban(), None, FleetPosture::Nominal),
        TrajectoryVerdict::Accept | TrajectoryVerdict::Clamp
    );

    let clear = plan_for_intent(&mut GeometricPlanner::default(), &intent, &world(ego, &route, &[], &g, goal));
    let follow = plan_for_intent(&mut GeometricPlanner::default(), &intent, &world(ego, &route, &lead, &g, goal));

    assert!(peak(&clear) > 5.0, "clear lane → drives on, peak {}", peak(&clear));
    assert!(peak(&follow) < peak(&clear) - 1.0, "the ego matches the slow lead, not cruise ({} vs {})", peak(&follow), peak(&clear));
    assert!(admit(&follow, &lead), "KIRRA admits the speed-matched follow");
...
fn the_lane_follow_mode_bounds_a_curving_in_merger_on_the_route() {
    ...
    let merge = LaneGraph::new().with_lane(lane(
        1,
        vec![Point { x_m: 12.0, y_m: 4.0 }, Point { x_m: 25.0, y_m: 4.0 }, Point { x_m: 35.0, y_m: 0.0 }, Point { x_m: 60.0, y_m: 0.0 }],
        0.0,
        &[],
    ));
    ...
    let ego = EgoState { pose: Pose { x_m: 5.0, y_m: 0.0, heading_rad: 0.0 }, linear_x_mps: 2.0, yaw_rate_rads: 0.0, stamp_ms: 0 };
    let goal = Pose { x_m: 80.0, y_m: 0.0, heading_rad: 0.0 };

    // CV-only (no lane graph): the object holds y=4, never enters the ego band → no yield.
    let w_cv = PlanInput { lane_graph: None, ..world(ego, &corr, &obj, &merge, goal) };
    ...
    assert!(reach(&cv) > 30.0, "CV: lane-parallel object → no yield, got {}", reach(&cv));
    assert!(reach(&mapped) < 28.0, "map-intention: the merging object is yielded to, got {}", reach(&mapped));
}
