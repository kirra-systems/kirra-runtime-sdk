...
use kirra_planner::{EgoState, GeometricPlanner, Goal, PlanInput, Planner, ProposalKind};
...
fn clear_corridor_nominal_stack_admits() {
    ...
    assert_eq!(
        kind,
        ProposalKind::Motion,
        "clear path → Occy proposes motion"
    );
    assert!(
        matches!(
            verdict,
...
fn lockedout_posture_flows_through_stack() {
    ...
    assert_eq!(
        ...
        "LockedOut → Occy proposes only safe-stop"
    );
    assert_eq!(
        verdict,
        TrajectoryVerdict::MRCFallback,
        "LockedOut → KIRRA refuses motion"
    );
}
...
fn route_around_in_corridor_object_admits() {
    ...
    perception
        .objects
        .push(kirra_trajectory::state::PerceivedObject {
            id: 99,
            pos: kirra_trajectory::corridor::Point {
                x_m: 20.0,
                y_m: 3.0,
            },
            velocity_mps: 0.0,
    ...
    assert!(
        min_y <= -1.0,
        "path offsets around the object, got min_y {min_y}"
    );
    assert!(
        matches!(
            verdict,
