//! Geometric doer — the `GeometricPlanner` reference planner.
//!
//! Extracted verbatim from the crate root (de-monolith): the config, the
//! planning body, the corridor/geometry helpers and the `LateralBump` shaper.
//! Behaviour is unchanged — `lib.rs` re-exports `GeometricPlanner`/
//! `GeometricPlannerConfig`, so all external paths are stable. The planner
//! PROPOSES; the checker (`kirra_trajectory`) remains the authority.
