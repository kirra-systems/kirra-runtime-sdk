...
1. `synchronous` policy per source class — measurement required. **Now
   ...
   **A policy is PROPOSED, not ruled**, in
   ...
   findings — that a per-class `synchronous` value is not implementable while
   `world_events` is the only writable table and one chain (the setting is
   per-connection); that batching is a 9.8× lever where the setting is 1.16× at
   batch=64; and that tier C's five power cuts (D-11) were run at `FULL`, so
