...
- The lane map carries a per-approach-lane **sight distance** (`LaneGraph::with_occluded_approach`),
  perception/map-supplied; absent ⇒ open view ⇒ no cap.
...
- **Honest scope:** the sight distance is integrator/map-supplied (it is not yet derived from
  occluder geometry on the lane graph); deriving it from a building/footprint model is a follow-up.
