# ADR-0015: Taj — the Rosmaster R2 perception layer (feeds the KIRRA perception input contract; geometric-first, then Parko ML)

| Field | Value |
|---|---|
| Status | **Proposed (design note)** — for owner sign-off; ratified on merge. |
| Name | **Taj** — the perception component. Stack: **Taj** (perception) → **Occy** (`kirra-planner`, planner) → **KIRRA** (governor / checker), with **Parko** (`parko-core`, vendor-neutral inference) as the substrate Taj's ML phase runs on. *Taj does perception; Parko is how Taj runs models.* |
| Date | 2026-06-21 |
...
## Context — Taj is a bounded, monitored INPUT to KIRRA, not free-form

KIRRA does not *compute* perception; it *consumes* it as a SEooC input (#126) that it bounds and
monitors. **Taj** is that perception layer. Three distinct consumers exist for the R2's raw
sensors, with **different trust semantics** — they must not be conflated (the blueprint / earlier
mapping blurred them):
...
The same lidar feeds all three; this ADR is **#3 — the safety-perception layer, named Taj**.

## The contract — what Taj must produce (read from the code)
...
`MockCorridorSource`. **Taj replaces the mock with the R2's real sensors.**

## Decision — Taj: R2 sensors → the contract, geometric-first
...
- **Taj Phase A — geometric, model-free (build first).** Lidar / depth → objects + corridor +
...
- **Taj Phase B — Parko ML detector (the "perception model").** RGB / depth detector → TensorRT
  via Parko → semantic classes. The safety plumbing is already proven by Phase A; this adds
...
- **Taj is not trusted blindly.** KIRRA *bounds and derates* it (checker over doer): a
  high-confidence-but-wrong perception still cannot exceed the kinematic envelope or re-initiate
  from a stop. Taj tightens the envelope; it never loosens it.
- **No raw-telemetry streaming to the console** (#394 out-of-mandate) — the console shows
  governance, not lidar / pose.
- **Full Autoware perception is not adopted.** Taj is the R2-scoped layer feeding the *same*
  contract Autoware's Option-B path feeds (#131) — interchangeable at the contract boundary.

## Status

**Proposed — for owner sign-off.** Taj (the ROS2 perception node) itself is laptop + hardware,
but this pins the build target (three outputs + health + world-frame + enablement gate) against
*existing* interfaces, so it is a **wiring job against a real contract, not research**. Sequenced
as ADR-0014 Phase 2; Taj Phase A (geometric) can start as soon as the R2 publishes `/scan` + odom.
