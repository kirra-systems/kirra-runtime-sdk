# Kirra Runtime SDK — Tier-1 ADAS Benchmark Production-Readiness Gap Analysis
...
**Question answered:** *"If engineers from the benchmark vendor evaluated kirra-runtime-sdk today, what capabilities would they conclude are missing before this could be considered production-grade?"*
...
- **[EST]** — reasoned estimate about what a benchmark-class evaluator would expect, deliberately avoiding claims about the vendor's proprietary implementations.
...
## 3. Benchmark Capability Matrix

Rating: ● present/strong · ◐ partial/prototype · ○ absent. "benchmark-class expectation" rows are [EST] anchored in [INDUSTRY] norms; "Kirra today" is [REPO].
...
| # | Gap | Sev | Why the benchmark vendor would expect it | Current state [REPO] | Desired state | Complexity / Effort | Dependencies | Risk if omitted |
|---|---|---|---|---|---|---|---|---|
...
- **The single biggest safety-case risk** is not any one artifact — it is that the *conditionality* of the whole case rests on AoUs (1,334 lines of them) that no current integration discharges: synchronized clocks, perception quality, localization integrity, dead-man base controllers. A benchmark-vendor reviewer would say: *the checker is real; the system it checks is hypothetical.* [REPO][EST]
...
Scores 0–10, [REPO]-grounded, [EST]-calibrated against "benchmark-class production" = 10 and "typical funded AV startup pilot" ≈ 5.
...
Where Kirra **exceeds** typical industry approaches (and would earn genuine respect in a benchmark-vendor review): [REPO][EST]
...
Where the benchmark vendor could reasonably view the architecture as **novel**: the combination of (2) + (3) — an LLM-era actuation firewall with a CI-enforced safety case — and the release-token-on-actuation-path design (ADR-0031) informed by an actual measured crypto-vs-WCET conflict. [EST]
...
**kirra-runtime-sdk is not production-grade by Tier-1-benchmark standards today, and it is one of the more credible foundations toward that bar that a small team could have built.**

A Tier-1 benchmark evaluation would likely conclude: [EST]
