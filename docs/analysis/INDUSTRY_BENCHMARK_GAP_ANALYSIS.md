...
| G4 | **Certified artifact produced by no pipeline** — QNX/Ferrocene governor is a documented recipe; release ships only the Linux verifier; all timing evidence VM-indicative | Critical | `release.yml`; `governor-service/README`; INDICATIVE-KVM labels | Medium | L | The thing being certified does not exist as a build product |
