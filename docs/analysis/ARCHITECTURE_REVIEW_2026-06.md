...
| ID | Title | File:line | Sev | Conf |
|---|---|---|---|---|
| CR1 | Reconciliation makes generation strictly authoritative *before* severity — a higher-gen `Nominal` peer masks a lower-gen `LockedOut` peer. **Bounded to the advisory `/federation/reports/:asset_id` read view; does NOT feed actuator-gating posture.** **DECISION (#692): generation-ordering KEPT as authoritative** (it is intentional — a stale `LockedOut` from a dead/lagging peer must not mask a fresh `Nominal` forever) **+ dissent overlay ADDED**: `dissenting_restriction` surfaces the most-restrictive *fresh* posture above the authoritative value in the response (`dissenting_restriction` field), so an operator is never blind to a live lockout. Additive, fail-safe, advisory-only. | `federation_reconciliation.rs:161` | Med | High |
