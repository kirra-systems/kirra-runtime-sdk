...
- **SDK:** Rust quickstart + examples (governed cmd_vel loop, action-filter integration, typed-intent planner); C example against an expanded `kirra.h` LANDED (verdict struct `KirraVerdict` + `kirra_check_move_velocity` → bounded scalar + stable `KIRRA_VERDICT_*` reason code; `kirra_posture()` → `KIRRA_POSTURE_*` operating-posture code; `kirra_envelope()` → the compiled `KirraEnvelope` linear/angular/accel bounds; `kirra_verify_release_token()` → the HVCHAN step-7 release-token verify over wire bytes with `KIRRA_RELEASE_*` codes — all exercised by the CI C example); Python client polish; published rustdoc + `deny(missing_docs)` on public crates.
