...
- Phase 3 (Stage 3–4): systematic ISO 21448 trigger catalog (weather, blooming, spray, low-sun, degraded markings, VRU edge cases, adversarial patches) with per-trigger scenario coverage and residual-risk ledger — trigger→evidence coverage is now machine-gated (`ci/sotif_trigger_coverage.json` via `scenario_kpi_gate`); adversarial-prompt eval suite for the Mick LLM seam LANDED (`crates/kirra-planner/tests/mick_adversarial_prompt.rs`: hostile-completion corpus proving the intent-vocabulary airgap + the checker backstop).
