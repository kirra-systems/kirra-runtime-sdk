...
- **SDK:** Rust quickstart + examples (governed cmd_vel loop, action-filter integration, typed-intent planner); C example against an expanded `kirra.h` (verdict struct + posture query LANDED — `KirraVerdict` + `kirra_check_move_velocity` returns the bounded scalar + a stable `KIRRA_VERDICT_*` reason code, and `kirra_posture()` returns a `KIRRA_POSTURE_*` operating-posture code, both exercised by the CI C example; still to add: envelope config, release-token verify); Python client polish; published rustdoc + `deny(missing_docs)` on public crates.
