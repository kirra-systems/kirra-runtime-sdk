...
| **MAR Phase 2** | QNX-native runtime | QNX process architecture, native IPC, bounded shared-memory profile (reusing the HVCHAN/iceoryx2 carrier discipline), service identities, QNX security policy, HAM integration, health aggregation, capability lifecycle, resource declarations. Console-observability attach points (KIRRA-MAR-CONSOLE-001 §4): health-event normalization (MAR-13), HAM event integration, APS telemetry collection (MAR-15), service/process-instance identity, bounded assurance stream contract (MAR-18) | Gate MA-C |
| **MAR Phase 3** | Physical safety boundary | Safety-gateway protocol, command envelopes, evidence binding, session epochs, replay protection, acknowledgements, watchdogs, HIL, independent controller integration, deployment verification. Console-observability attach points: authority/safety-plane consequence correlation (MAR-17), restart/epoch binding (MAR-13 on MAR-06), fault-containment evidence (MAR-16) | Gate MA-D |
| **MAR Phase 4** | Production supervision | Signed packages, device identity, anti-rollback (Uptane reuse), update/rollback, deadline monitoring, declared degraded modes, operator approvals, ROS 2 integration, evidence export, offline operation. Console-observability attach points: console recovery feed (MAR-14), APS budget cards (MAR-15), fault-containment views (MAR-16), operator-facing evidence links (MAR-19), production authorization for console access (MAR-18) | Gate MA-E |
| **MAR Phase 5** | Replay and assurance productization | Replay inspection, divergence reports, counterfactual prototype, policy comparison, capability/model substitution, incident reports, assurance exports. Console-observability attach points: exact replay of QNX assurance events + deterministic console reconstruction (MAR-20), recovery-duration/containment reports, deadline+partition evidence correlation, incident export | (feeds Gate MA-F) |
