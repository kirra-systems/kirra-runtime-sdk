...
| Sn-INTEG | WCET / MC-DC / FFI / traceability | `docs/safety/GOVERNOR_INTEGRITY_EVIDENCE.md`, `src/wcet_gate.rs`, `OCCY_MCDC_EVIDENCE.md`, `OCCY_FFI_EVIDENCE.md` | **Present**; WCET proven (p99.9 ≈ 170–352 ns), MC/DC ≈ 96%+ on critical paths |
...
| SPI-L04 | Perception-derate rate | L | Detection range / snapshot health below floor | `PERCEPTION_SNAPSHOT_UNHEALTHY`, `DETECTION_RANGE_DEGRADED` | ↓ | EMITTED (`perception_monitor`) |
