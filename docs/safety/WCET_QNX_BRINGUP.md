...
passes byte-identically (`GATE: PASS`, all 9 rows), and **`max = 19.96 µs < 100 µs`**
...
| representative `SCHED_FIFO` WCET (`max < 100 µs`, acceptance #4) | **met (KVM-INDICATIVE, 2026-06-30)** — `max = 19.96 µs` under `SCHED_FIFO` on the KVM VM (`tools/qnx-rtm-harness/results/qnx800-x86_64-vm-kvm.txt`; ~100× cleaner than TCG's 2.20 ms). A VM figure is never cert-grade: the row self-declares `INDICATIVE-NOT-WCET`. Cert-grade is Phase-II (DRIVE + QNX OS for Safety) |
