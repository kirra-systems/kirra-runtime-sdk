...
| AOU-ACTUATION-SERIAL-001 | On the `ros2_ws`/R2 deployment topology, the actuator serial device (Rosmaster expansion board) is opened ONLY by the verifying motor-consumer process (consumer user, mode 0600); no other process on the host can reach the port | Integrator (deployment / OS config) | **ENFORCED (#887 Tier-3)** — the consumer's boot sentinel (`robot/serial_exclusivity.py`) REFUSES startup on wrong owner / loose mode / another holder (`KIRRA_ALLOW_SHARED_SERIAL=1` = loudly-labeled bring-up only, no authority claim); TIOCEXCL holds kernel exclusivity for the session; `99-kirra-serial-exclusivity.rules` (installed by `install_kirra.sh`) replaces the vendor's world-writable 0777 rule; `kirra_doctor` devices census FAILs on drift. Residual integrator half: install the rule, run the consumer non-root, don't grant other processes root | PO-2 independence on the ROS topology (`OCCY_DFA.md` §3 scope note): with an unowned port, any process can actuate below the bus, bypassing the checker entirely |
...
| AOU-METRICS-SEGMENTATION-001 | `GET /metrics` is unauthenticated and posture-exempt: it discloses fleet posture, HA topology (`mode_active`), instance identity, failover/abort counts, and per-reason denial counts to any peer that can reach it. The integrator EITHER enables the dedicated ops listener (`KIRRA_METRICS_ADDR` bound to an ops/management interface — the command-plane port then 404s `/metrics`, ending its exposure there) OR, with the listener disabled, restricts command-plane scrape reachability to a trusted operations segment | Integrator (deployment / network) | **MECHANISM LANDED (#1123)** — `KIRRA_METRICS_ADDR` serves `/metrics` on its own listener with its own shed pool (move-not-copy; invalid value = startup abort; #793 F6/F7/F8 hardened the content). Default remains off for compatibility, so the residual integrator half is: enable it, and keep the ops segment trusted (the listener is deliberately plaintext — TLS is the command listener's concern) | the confidentiality of fleet-operational state (posture / HA role / denial telemetry); the industry norm of diagnostics on a dedicated ops port, not the command-plane application port |
...
| AOU-WM2-STORAGE-COMPLETION-001 | The block device carrying the World Model store does not lose I/O completion interrupts, so commit latency is bounded by device service time rather than by the driver's command-timeout recovery path. Where that cannot be assured for a drive/firmware/host-controller combination, the integrator qualifies the device by measurement and bounds the worst case via `nvme_core.io_timeout` | Integrator (hardware / platform) | **OPEN — MEASURED VIOLATED** on the WM-2 bench Jetson (D-15): 5 stalls in 120 repetitions, each a lost NVMe completion resolved by the 30 s timeout handler finding the command already complete. Root cause (firmware / PCIe ASPM / Tegra MSI-X) not discriminated. Durability unaffected — every command had completed | WM-2 memory coverage: at 10 Hz a 30 s stall is ~300 observations that cannot be recorded at the 30 s bound, and a shorter unbounded-by-timeout variant also observed (8.5 s), with nothing in the store marking either gap. Severity is contingent on ADR-0042 Decision 5 — see the entry's *Status / scope* |
...
- **Separate ops listener — LANDED (#1123, closing #793 F2).** Opt-in
  `KIRRA_METRICS_ADDR`: when set, `/metrics` is served on a dedicated listener
  (its own shed pool, `KIRRA_METRICS_MAX_CONCURRENCY`, default 16) and the
  command-plane port answers `/metrics` with 404 + a pointer — MOVE, not copy,
  so the exposure this AoU records actually ends there. A set-but-invalid
  address aborts startup (never a silent fallback to the exposed port); the
  ops listener serves ONLY `/metrics` (no command-plane surface can grow onto
  the ops port by accident). Unset keeps today's behavior, so the residual
  obligation is simply to ENABLE it (or keep the segmentation restriction).
...
- **Mitigation status on the bench, 2026-08-04.**
  - **`io_timeout` — ATTEMPTED AND NOT IN EFFECT.** Writing
    `/sys/module/nvme_core/parameters/io_timeout` at runtime set the module
    parameter to 5, but the **live queue was unchanged at 30 000 ms**
    (`/sys/block/nvme0n1/queue/io_timeout`). The value is latched when the
    namespace is probed, so a runtime write to the module parameter never
    reaches an existing device. **This is recorded because an earlier revision
    of this entry claimed the mitigation was applied; it was not.** The knobs
