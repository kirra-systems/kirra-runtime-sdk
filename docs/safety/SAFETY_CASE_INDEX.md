...
| KIRRA-OCCY-ASIL-DECOMP-001 | Consolidated ISO 26262-9 ASIL decomposition work product (navigable synthesis of DFA + SGs + FFI + fault model + timing + AoU) | 0.1 | Draft | docs/safety/ASIL_DECOMPOSITION.md | 2026-06-30 |
...
| KIRRA-OCCY-WCET-METH-001 | WCET measurement methodology (#274/#279 timing-evidence strategy; "measuring is not proving"). Declares the evidence class = MEASUREMENT-BASED TIMING ANALYSIS (high-water-mark under engineered worst-case load + justified margin), NOT static WCET (no qualified static-WCET tool; paths O(1)/loop-free per `src/wcet_gate.rs`); margin VALIDATION-PENDING (set with the FTTI budget); honest class limit (measured = lower bound; static-analysis escalation for highest-ASIL named as a known possibility). The six assessor questions answered per-environment (Governor-QNX = tracelogger/SAT under FIFO + normative observer-effect rule; Guest-Ubuntu LTTng/perf = NON-evidence, diagnostic only); worst-case-load definition tied to the #279 catalogue (one campaign, not two); measurement protocol (warmup, p50/p99/p99.9/max, repeatability bound, environment freeze, host-numbers-never-WCET). Precedent reconciliation: the S3 gateway p99.9 ≈ 170–352 ns is CI/host-measured — structural boundedness CONFORMS, the NUMBERS are RE-MEASURE-UNDER-METHODOLOGY on QNX-under-FIFO (#274), not grandfathered. DRAFT — pending safety-engineer review | 0.1 | Draft — pending review | docs/safety/WCET_MEASUREMENT_METHODOLOGY.md | 2026-06-11 |
| KIRRA-OCCY-ADR-033 | ADR-0033: Actuation authority on the ROS/R2 deployment topology — verify-before-release at the motor boundary. Records the PO-2 topology gap (bus open + serial device unowned on `ros2_ws`; DFA scope note), the threat model, the (i) crypto-chokepoint-then-(ii) sros2 decision extending ADR-0031 to the ROS boundary, the three settled implementation decisions (FFI-Rust consumer verify core; separate `KIRRA-ROS-*-V1` signing domains; resync-from-zero + token freshness window), the SS-002-shaped motor-side safe state, and the Tier-1/2/3 regression guard (planned evidence: rogue-publisher cross-process test over a mock serial seam + launch-level no-move drill + boot-time port-ownership sentinel) | 0.1 | Proposed — pending owner sign-off | docs/adr/0033-actuation-authority-ros-r2-topology.md | 2026-07-09 |
| KIRRA-R2-ODD-001 | R2 Operational Design Domain specification (#1220). The first ODD authored for the Rosmaster R2 bench platform — previously its entire ODD was one parenthetical inside `R2_ODD_SPEED_CAP_MPS`'s doc comment ("a small indoor/tethered Ackermann robot"). Authored, not extracted: `OCCY_SOTIF.md` §1 holds the urban/robotaxi ODD and ADR-0028 the courier ODD, and neither is this platform's. Each boundary is a checkable condition classified ENFORCED / BY CONSEQUENCE / ASSUMED, with §4 stating plainly that most boundaries are not detected — every enforced boundary on the platform is an EVIDENCE check (scan freshness, cap staleness, request deadline, confidence floor, envelope), and every ENVIRONMENTAL boundary is an operator obligation. Raises five assumptions of use (AOU-R2-ENVIRONMENT/SURFACE/CROWD/SUPERVISION/LIDAR-HEALTH-001), of which SUPERVISION is the platform's largest residual risk: the software-independent hardware e-stop is specified (`R2_ESTOP_SPEC.md`) and NOT built, so the stop of last resort is a software stop. DRAFT — pending safety review | 0.1 | Draft — pending review | docs/safety/R2_ODD.md | 2026-07-29 |
| KIRRA-R2-RESIDUAL-001 | R2 residual-risk register (#1220). Keyed by HARA hazard PLUS residual scenario (`H-NNN / R2-RS-NNN`), because one hazard can carry mitigations leaving materially different residuals — H-006 is the case, where ADR-0033's token chokepoint eliminates the original causal path while the wedged-process path remains. Full HARA walk: all 17 hazards get exactly one disposition (5 INAPPLICABLE with named absent subsystems, 5 MITIGATED no-material-residual, 2 ELIMINATED by architecture, 6 RESIDUAL). Every row carries explicit DETECTION behaviour and credited-mitigation STATUS, so a specified-but-unbuilt control is credited at zero. FINDING: the completeness assertion 'every residual resolves to a HARA hazard' FAILS — HARA.md's 17 hazards are all MALFUNCTION hazards, and the R2's three largest environmental risks (stair, unbriefed person, outdoors) are SOTIF-shaped: the software functions exactly as designed against inputs that are complete and wrong. Recorded as ODD-derived rows with the assertion restated; extending HARA.md is flagged as a safety-review action, not taken here. NO ROW IS ACCEPTED — HARA.md names no acceptance authority. DRAFT — pending safety review | 0.1 | Draft — pending review | docs/safety/R2_RESIDUAL_RISK.md | 2026-07-29 |
