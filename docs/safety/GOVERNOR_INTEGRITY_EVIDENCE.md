...
**What justifies keeping it is measured solver behaviour, and the measurement is genuinely mixed.** CBMC's `--verbosity` does not reach an external solver, so this needed kissat's own telemetry captured through a pass-through wrapper on `--external-sat-solver`. Over a full 43-minute solve:

| phase | conflicts | rate | "remaining" |
|---|---|---|---|
| 0 → 376 s | 26,000 | 70/s | 97% → 29% |
| 376 → 2,604 s | 94,659 | 42/s | 29% → 26% |
| **total** | **120,888** | — | no verdict |

Read it in both directions, because it supports both and neither cleanly. **The search never stalls** — conflicts accumulate steadily throughout, restarts and reductions keep advancing, and backbone probing, vivification and substitution stay active. But **instance-reduction progress decelerates sharply**: nearly all of it happens in the first six minutes, and the following thirty-seven minutes buy three further percentage points. Two independent clause-count columns decline monotonically across the run (≈930k → 608k, and ≈135k → 65k), so simplification does continue — just far more slowly.

So R2 sits **between** "progressing" and "plateaued" rather than in either. What that supports is keeping it as a budget/solver question — the search is alive, and solver choice or a portfolio approach has a plausible path. What it does NOT support, and must not be written up as, is a claim that any particular budget suffices: 120,888 conflicts in 43 minutes is slow, the deceleration is real, and progress is not a termination guarantee. For R2 the concrete mirror remains the only tier, and the standing rule that every deep harness keeps a blocking per-PR mirror is the sole reason that is a coverage REDUCTION rather than a HOLE.
