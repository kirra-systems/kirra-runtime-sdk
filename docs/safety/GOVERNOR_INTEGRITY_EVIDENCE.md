...
**On R2, the wording is deliberate.** Its 300-minute isolated timeout excludes the old starvation explanation, but it is NOT a finding of intractability and this document must not be edited to say so. R2 is the one harness that never stalls in conversion: it reaches the solver and stays there, on a formula **4× smaller** than K7's or K8's (2,227 program-expression steps and 74 VCCs, against ~8,900 and ~400). Its expensive solve is the SECOND one, after a cheap 36 s SAT — the UNSAT direction, the ordinary shape of a hard-but-not-impossible instance. That is a solver question (budget, solver choice, portfolio), not a modelling one, and it has a live answer. Whether the solver is making progress or plateauing is NOT currently observable — CBMC's `--verbosity` does not pass through to an external solver — so the branch is genuinely open rather than quietly assumed. For R2 the concrete mirror remains the only tier, and the standing rule that every deep harness keeps a blocking per-PR mirror is the sole reason that is a coverage REDUCTION rather than a HOLE.
