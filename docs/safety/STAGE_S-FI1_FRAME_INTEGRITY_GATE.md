...
S-FI1e: ✅ DONE — shim collapsed (4-arg `validate_trajectory_containment` is canonical); `frame_trust` threaded through `validate_trajectory_slow_capped` and the production node (Trusted = AOU-LOCALIZATION-001 seam until a real source is wired); convenience wrapper + ~50 doer callers untouched. Live-gate adapter test passes. **Remaining:** S-FI1f (RTM/AoU updates + ADR-0016) before any ENFORCED claim.

S-FI1e-source: ✅ DONE — the node-side integrator contract is now WIRED. `AdaptorState::update_frame_integrity(report, now_ms)` feeds the per-tick `FrameIntegrity`; `AdaptorState::snapshot_frame_trust(now_ms)` resolves the LIVE `FrameTrust` the slow loop passes to the gate (replacing the hardcoded `Trusted`). Three regimes, all fail-closed: **no source ever reported** → `Trusted` (the AoU seam — byte-for-byte the prior behaviour, backward-compatible); **a once-live source gone silent** past `max_age_ms` → `Untrusted`; **a fresh report** → `resolve_frame_trust` (Trusted/Degraded/Untrusted by ε, with non-finite-ε / report-internal-staleness fail-closed). 3 state tests + adapter suite green. **Remaining for the integrator:** call `update_frame_integrity` from the localization/pose-quality stack (the contract this seam exposes); independent KIRRA-computed integrity is still S-FI3.
