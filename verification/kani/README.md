...
| `proofs_kinematics` | `crates/kirra-core/src/kinematics_contract.rs` — the FROZEN talisman (git blob `851f3f44…`), proved **unmodified** | K1 NaN/Inf fail-closed totality (every f64 bit pattern, every field) · K2 non-positive dt denied · K3 P2 speed ceiling — the return is the ceiling exactly or a value the rate bound explains (restated #1243; **`deep-proofs` — SG1 lost its per-PR symbolic proof, an assurance change, see the harness**) · K3b composed enforcement reports both axes · K4/K5 Degraded re-initiation / speed-increase denials (#70) · K6 (P-CAP) every executable return respects the speed ceiling · K7 (P-RACK) every executable return respects the rack limit (`deep-proofs`) · K8 every executable return respects the accel/brake rate bound, in velocity space (#1243, `deep-proofs`) |
