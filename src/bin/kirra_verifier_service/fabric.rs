...
pub(crate) async fn handle_register_fabric_asset(
    ...
    if req.asset_id.trim().is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "asset_id is required"})),
        )
            .into_response();
    }
    ...
    (
        StatusCode::CREATED,
        Json(json!({"asset_id": req.asset_id, "registered": true})),
    )
        .into_response()
}
...
pub(crate) async fn handle_fabric_state(State(svc): State<Arc<ServiceState>>) -> impl IntoResponse {
    ...
    let changes = svc
        .fabric_router
        .propagate_and_record(&svc.fabric_causal_log, fabric_generation);
...
pub(crate) async fn handle_fabric_causal_log(
    ...
    Json(json!({"entries": entries, "total": total, "limit": limit, "offset": offset}))
        .into_response()
}
...
pub(crate) async fn verify_causal_chain(State(svc): State<Arc<ServiceState>>) -> impl IntoResponse {
    ...
    match svc
        .app
        .store
        .call_read(move |store| store.verify_causal_chain_integrity(vk.as_ref()))
        .await
    {
        Ok(Ok(r)) => Json(json!({
        ...
        _ => (
            ...
            Json(json!({ "error": "causal chain query failed" })),
        )
            .into_response(),
    }
}
