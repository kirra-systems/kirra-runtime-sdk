...
fn watchdog_sweep_once_inner(
        ...
        || *last_node_refresh_ms == 0
    {
        ...
        let load_result = app.store.shared().load_all_registered_av_node_ids();
    ...
    for entry in node_health.values_mut() {
        ...
        if let Ok(ts) = app
            .store
            .shared()
            .get_last_telemetry_timestamp(&entry.node_id)
        {
        ...
        if silence_ms >= AV_TELEMETRY_TIMEOUT_MS {
            ...
            if !already_timed_out {
                ...
                let _ = app
                    .store
                    .shared()
                    .reset_recovery_streak_preserving_telemetry(&entry.node_id);
