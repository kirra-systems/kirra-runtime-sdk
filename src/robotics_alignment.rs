...
impl<C: SafetyContract + Copy> AlignmentBridge<C> {
    pub fn new(contract: C) -> Self {
        ...
        let ros2_adapter = Ros2Adapter::new(angular_limit).unwrap_or(Ros2Adapter {
    ...
    pub fn align_and_serialize_intent(
        &self,
        raw_json: &str,
    ) -> Result<(FilterOutput, Vec<u8>), &'static str> {
        let action = self.parser.parse_llm_json_intent(raw_json)?;
        let contract = self.filter.contract;
        let mut gov =
            KirraKernelGovernor::new(contract, 0.0, contract.min_bound(), contract.max_bound());
