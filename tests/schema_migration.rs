...
fn pre_framework_database_upgrades_in_place_without_data_loss() {
    ...
    // as an older binary that never used user_version would have left it.
    {
        ...
        s.register_cert_principal("svc", FP, "auditor", None, 1_000)
            .expect("write");
    }
    ...
    assert_eq!(
        s.schema_version().unwrap(),
        SCHEMA_VERSION,
        "version-0 DB upgraded to baseline"
    );
    let rec = s
        .load_cert_principal_by_fingerprint(FP)
        .unwrap()
...
fn future_schema_database_is_refused_fail_closed() {
    ...
    assert!(
        err.is_err(),
        "a future-schema database must be refused, not opened"
    );
