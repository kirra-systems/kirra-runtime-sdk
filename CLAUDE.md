...
| `crates/kirra-taj` | **Taj, the R2 perception layer (ADR-0015).** Phase-A geometric corridor/objects from lidar; Phase-B semantic fusion (`clip_corridor_to_hazards` / `binding_hazard` / `hazard_clip_x` — water/obstacle hazards tighten the corridor); **`SemanticEvalSummary`** — the safety-weighted perception eval harness (`UnsafeMiss` / `OverConservative` / `Correct`, `hazard_recall`). EP-20: `kirra_kpi_gate::differential` runs Phase-A vs Phase-B over the shared ground-truth corpus and the KPI gate carries differential rows — `MissedTighten` (required divergence absent, unsafe) / `PhantomTighten` (unjustified, availability) / `ForbiddenLoosen` (fusion may NEVER extend Phase-A; hard 0) — with per-fault-family negative controls that must breach. **#1210 `self_filter`** — the ONE operation in Taj that REMOVES evidence (masking a return ADDS free space), so its safety argument runs backwards from the rest of the crate and every constraint bounds how much a mask can destroy: validated-at-construction `SelfFilterMask` (fail-closed refusal, never a silent clamp), a vertex-radius bound derived from the platform footprint (`SelfFilterBounds::for_footprint`, backstopped at `MAX_SELF_MASK_VERTEX_RADIUS_M` = 1.0 m), on-boundary returns RETAINED, masking applied in the SENSOR frame before any transform, a `MountProvenance` gate (`assert_mount_verified` refuses anything but `Measured`), a `digest_hex()` so a geometry change is a digest change, and `MAX_SELF_MASKED_RAY_FRACTION` (0.5) so a mask cannot buy confidence by shrinking `scan_confidence`'s denominator. `ingest_scan` is the single funnel and returns a per-reason `RejectionTally` (NaN split from the ROS `+inf` no-return convention) that rides `TajPerception.rejected` to the sidecar wire as observability, deliberately OUTSIDE the evidence digest. DEFAULT DISARMED (`self_filter: None`, byte-identical) — the physical capture that says which returns are the robot has not been run; procedure in `docs/hardware/R2_SELF_FILTER_CALIBRATION.md`. |
