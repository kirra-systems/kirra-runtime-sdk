...
| `crates/kirra-ota-installer` | **WS-4/Track-3 node-side dual-slot (A/B) governor-artifact installer** (doer side). Device-AGNOSTIC core: `Installer<B>` slot state machine (`Idle→Staged→Trying→{commit\|retry\|rollback}`), fail-closed `verify_staged_artifact` (SHA-256 vs the campaign's signed digest — a mismatch never arms the slot), health-gated automatic rollback, and the `BootController` trait (the one hardware seam: `InMemoryBootController`/`FileBootController` here; the Jetson `nvbootctrl`/EFI impl is the rootfs-level follow-up). **App-level A/B** is live end-to-end: the `kirra-ota-ctl` bin (systemd `ExecStart=… run` launcher enforcing one-shot `try_boot` + `stage`/`commit`/`rollback`/`status`, pure `plan_*` transitions over `BootRecord{active,try_boot,trying}`) + a systemd unit + the `docs/ota/ORIN_APP_LEVEL_AB_DRILL.md` runbook. Consumes the verifier's `/fleet/campaigns/assignment/{node_id}` (#829) via a thin HTTP agent (follow-up). |
